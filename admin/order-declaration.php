...
$items = $pdo->query("SELECT oi.*, COALESCE(oi.unit_price, p.price, 0) as unit_price_safe FROM order_items oi LEFT JOIN products p ON oi.product_id = p.id WHERE oi.order_id = $id")->fetchAll();
        ...
        <table>
            ...
            <tbody>
                ...
                foreach ($items as $idx => $item):
                    $uPrice = $item['unit_price_safe'] ?? $item['unit_price'] ?? $item['price'] ?? 0;
                    $uSub = $item['subtotal'] ?? ($uPrice * $item['quantity']);
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <?php echo number_format($uPrice, 2, ',', '.'); ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
