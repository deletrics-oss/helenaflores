...
if (isset($_GET['ajax_quote']) && isset($_GET['order_id'])) {
    ...
    // Check if user forced custom dimensions via UI
    ...
    $products = [
        [
            ...
            'length' => $finalL,
            'weight' => $finalWt,
            'insurance_value' => (float)$order['total_amount'],
            'quantity' => 1
        ]
    ];
    
    $result = $me->calculateShipping($fromZip, $order['dest_zip'], $products);
    echo json_encode(['quotes' => $result, 'order' => $order]);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_buy'])) {
    ...
    // Capture custom dimensions from POST if available (passed from the UI)
    ...
    if (isset($cartResult['id'])) {
        ...
        // Save to our DB
        try {
            // Even if not paid, save me_order_id so we can sync later
            $pdo->prepare("UPDATE orders SET me_order_id = ?, shipping_method = ? WHERE id = ?")
                ->execute([$meOrderId, 'Melhor Envio', $oid]);
        } catch (Exception $e) { }
...
if (isset($_GET['ajax_sync']) && isset($_GET['order_id']) && isset($_GET['me_id'])) {
    ...
    if (isset($res[$me_id]['tracking'])) {
        ...
        $pdo->prepare("UPDATE orders SET tracking_code = ?, status = 'shipped' WHERE id = ?")->execute([$track, $oid]);
        echo json_encode(['success' => true, 'tracking' => $track]);
    } else {
...
<body>
    ...
    <div class="container" style="padding-top:2rem;">
        ...
        <div class="balance-box">
            ...
            <span class="balance-val">R$ <?php echo number_format($balance['balance'] ?? 0, 2, ',', '.'); ?></span>
        </div>
        <a href="https://melhorenvio.com.br/painel/gerenciar/carteira" target="_blank" class="btn" style="background:#2ecc71; color:#000; margin-left:10px; font-weight:bold; padding:10px 15px; margin-bottom:1rem; display:inline-block; vertical-align:top;">➕ Adicionar Saldo (PIX)</a>
        <?php endif; ?>
        <?php else: ?>
    ...
    function openQuote(orderId) {
        ...
        fetch(`melhorenvio.php?ajax_quote=1&order_id=${orderId}`)
            ...
            .then(data => {
                ...
                if (data.error) {
                    document.getElementById('quoteResults').innerHTML = `<div style="color:#e74c3c;padding:1rem">${data.error}</div>`;
