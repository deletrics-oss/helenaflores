...
if (isset($_GET['ajax_send_wa']) && isset($_GET['id'])) {
    ...
    if ($order && !empty($order['phone'])) {
        ...
        $res = $notif->orderShipped($order['phone'], $order['customer_name'], $oid, $track);
...
if (!empty($_POST['selected_orders'])) {
    ...
    if (isset($_POST['bulk_delete_orders'])) {
        $idList = implode(',', array_map('intval', $ids));
        $pdo->query("DELETE FROM orders WHERE id IN ($idList)");
        header("Location: orders.php?msg=bulk_deleted");
        exit;
    }
    
    if (isset($_POST['bulk_notify_whatsapp'])) {
        require_once __DIR__ . '/../includes/notifications.php';
        $notif = new NotificationService($pdo);
        $count = 0;
        foreach ($ids as $oid) {
            ...
            if ($order && !empty($order['phone'])) {
                $track = !empty($order['tracking_code']) ? $order['tracking_code'] : ($order['me_order_id'] ?? '');
                $notif->orderShipped($order['phone'], $order['customer_name'], $oid, $track);
                $count++;
            }
        }
    ...
    <div class="container" style="padding-top:2rem;">
                ...
                fetch(`orders.php?ajax_send_wa=1&id=${id}`)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            alert('Erro: ' + (data.error || 'Verifique o WhatsApp'));
                            setTimeout(() => {
                                btn.innerHTML = originalHtml;
        ...
        <div class="debtors-bar">
            ...
            <div class="debtors-stats">
                ...
                <div class="debtors-stat">
                    <div class="val" style="color:#e74c3c">R$ <?php echo number_format($totalPendingValue, 2, ',', '.'); ?></div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if (!empty($o['tracking_code'])): ?>
                                        <a href="https://www.melhorrastreio.com.br/rastreio/<?php echo $o['tracking_code']; ?>" target="_blank" style="text-decoration:none;">
                                            <div style="margin-top:5px; font-size:0.7rem; background:#000; padding:3px 5px; border-radius:4px; color:var(--primary); font-family:monospace; border:1px solid #333; text-align:center; box-sizing:border-box;">
                                ...
                                <td>
                                    <div style="display:flex; gap:5px; margin-top:5px; flex-wrap:wrap;">
                                        ...
                                        ?>
                                        <?php if (!empty($me_oid) && !$is_lalamove): ?>
                                            <?php if (!$has_track): ?>
        ...
        <div id="llmModal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.85);z-index:9999;align-items:center;justify-content:center">
            <div style="background:#1a1e26;border:2px solid #FF6600;border-radius:16px;padding:2rem;max-width:600px;width:90%;max-height:80vh;overflow-y:auto">
                ...
                <div id="llmActions" style="display:none;margin-top:1rem;text-align:right">
                    ...
                    <label style="display:inline-flex;align-items:center;gap:8px;margin-bottom:10px;color:#bbb;font-size:0.9rem;cursor:pointer;width:100%">
                        <span style="flex:1">Tipo de Entrega:</span>
                        <select id="llmDeliveryTier" onchange="document.getElementById('llmPriorityFeeGroup').style.display=this.value==='priority'?'flex':'none'" style="background:#222;color:#fff;border:1px solid #444;padding:5px;border-radius:4px;">
