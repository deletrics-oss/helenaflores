...
if (isset($_GET['ajax_quote'])) {
    ...
    // Se o usuário quer pagamento em dinheiro (COD), passa o specialRequest
    $specialRequests = [];
    if (isset($_GET['cod']) && $_GET['cod'] === '1') {
        $specialRequests[] = 'COD';
    }

    $quotes = $llm->getAllQuotations(['lat' => $lat, 'lng' => $lng], $address, $specialRequests);
...
<body>
            ...
            <div id="passo1">
                ...
                <div class="form-group">
                    ...
                    <select id="delivery_tier" onchange="togglePriorityFee()" style="background:#0d1017;color:var(--text);border:1px solid var(--border);border-radius:8px;padding:9px 11px;width:100%">
                        ...
                        <option value="priority">⚡ Prioridade (gorjeta para motorista aceitar mais rápido)</option>
                    </select>
                </div>
    ...
    function goToQuote() {
        ...
        const codParam = payMethod === 'CASH' ? '&cod=1' : '';

        fetch(`lalamove.php?ajax_quote=1&lat=${lat}&lng=${lng}&address=${encodeURIComponent(addr)}${codParam}`)
            .then(r => r.json())
            .then(data => {
    ...
    // Toggle visibility da gorjeta de prioridade
    function togglePriorityFee() {
        document.getElementById('priorityFeeGroup').style.display = 
            document.getElementById('delivery_tier').value === 'priority' ? 'block' : 'none';
    }
