...
/**
 * admin/lalamove.php
 * Painel de integração Lalamove — Fight Arcade
 * Entrega expressa intra-cidade (São Paulo / Rio)
 */
...
if (isset($_GET['ajax_geocode'])) {
    ...
    if (empty($cep) && !empty($_GET['order_id'])) {
        ...
        $orderUser = $pdo->prepare("SELECT u.zipcode, u.address, u.number, u.city, u.state, u.neighborhood 
...
if (isset($_GET['ajax_quote'])) {
    ...
    $quotes = $llm->getAllQuotations(['lat' => $lat, 'lng' => $lng], $address);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_create_order'])) {
    ...
    $result = $llm->createOrder($quotationId, $stops, $recipientName, $recipientPhone, $remarks, $notifySms, $paymentMethod, $totalValue);
...
if (isset($_GET['ajax_status']) && isset($_GET['order_id'])) {
    ...
    echo json_encode([
        ...
        'driverName'   => $result['data']['driverInfo']['name']        ?? '',
        'driverPhone'  => $result['data']['driverInfo']['phone']       ?? '',
        'driverPlate'  => $result['data']['driverInfo']['plateNumber'] ?? '',
    ]);
    exit;
}
...
<body>
    ...
    function goToQuote() {
        ...
        fetch(`lalamove.php?ajax_quote=1&lat=${lat}&lng=${lng}&address=${encodeURIComponent(addr)}`)
            .then(r => r.json())
            .then(data => {
                ...
                currentQuotes.forEach((q, i) => {
                    ...
                    const icon = { MOTORCYCLE:'🏍️', VAN:'🚐', TRUCK175:'🚛', TRUCK330:'🚛' }[q.serviceType] || '📦';
                        ...
                        <div class="quote-card ${hasError ? 'unavailable' : ''} ${isCheap ? 'cheapest' : ''}"
                            ...
                            <div class="quote-info">
                                ...
                                ${hasError
                                    ...
                                    : `<div class="quote-meta">Entrega expressa no mesmo dia</div>`
                                }
                            </div>
    ...
    function selectQuote(idx) {
        document.querySelectorAll('.quote-card:not(.unavailable)').forEach(c => c.classList.remove('selected'));
        const cards = document.querySelectorAll('.quote-card:not(.unavailable)');
        const q = currentQuotes.filter(x => !x.error);
        let clickIdx = 0;
        document.querySelectorAll('.quote-card').forEach((c, i) => {
            if (!c.classList.contains('unavailable')) {
                if (currentQuotes.indexOf(currentQuotes.filter(x=>!x.error)[clickIdx]) === idx) {
        ...
        // Simpler: mark by idx in currentQuotes
        document.querySelectorAll('.quote-card').forEach((c, i) => {
        ...
        document.getElementById('confirmSummary').innerHTML = `
            <div style="background:rgba(255,255,255,0.03);border:1px solid var(--border);border-radius:12px;padding:1.25rem">
                ...
                <div style="display:flex;justify-content:space-between;margin-bottom:.5rem">
                    ...
                    <span style="font-weight:600">${document.getElementById('payment_method').value === 'CASH' ? '💵 Dinheiro (Local)' : '💳 Carteira'}</span>
                </div>
    ...
    function checkStatus(llmOrderId, localId) {
        fetch(`lalamove.php?ajax_status=1&order_id=${llmOrderId}`)
            ...
            .then(data => {
                ...
                if (data.driverName)  info += `<br>Motorista: ${data.driverName} (${data.driverPhone})`;
                if (data.driverPlate) info += ` — Placa: ${data.driverPlate}`;
                alert(`Status: ${statusLabel(s)}\n${data.driverName ? 'Motorista: ' + data.driverName + ' ' + data.driverPhone : ''}`);
            });
    }
