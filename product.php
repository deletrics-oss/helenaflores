...
if ($id || $slug) {
    ...
    if ($isAdmin) {
        $stmt = $pdo->prepare("SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE $where");
    } else {
        $stmt = $pdo->prepare("SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE $where AND p.active = 1 AND (p.show_on_site = 1 OR p.show_on_site IS NULL) AND (c.show_on_site = 1 OR c.show_on_site IS NULL OR p.category_id IS NULL)");
    }
    $stmt->execute([$param]);
    $product = $stmt->fetch();
}
...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            .product-container { display: flex; flex-direction: column; gap: 20px; }
            .product-gallery { width: 100% !important; }
            .product-info { width: 100% !important; }
            .main-img img { max-height: 350px !important; object-fit: contain; }
            ...
            <div class="product-details-box">
                ...
                <form action="cart.php" method="POST"
                    ...
                    <div style="display:flex; gap:15px; margin-top:1.5rem; align-items:center;">
                        <input type="number" name="qty" value="1" min="1" id="qtyInput"
