...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        try {
            $sql = "UPDATE products SET name=?, sku=?, ean=?, ncm=?, description=?, price=?, price_wholesale=?, min_wholesale_qty=?, category_id=?, image_path=?, video_url=?, is_vip=?, is_manufactured=?, weight_kg=?, length_cm=?, width_cm=?, height_cm=?, seo_title=?, seo_description=?, brand=?, gtin=?, mpn=?, condition_status=?, stock_qty=?, show_on_site=?, allow_export=? WHERE id=?";
            $pdo->prepare($sql)->execute([$name, $sku, $ean, $ncm, $desc, $price, $price_w, $min_w, $cat_id, $image_path, $video_url, $is_vip, $is_manufactured, $weight, $len, $width, $height, $seo_title, $seo_desc, $brand, $gtin, $mpn, $condition, $stock_qty, $show_on_site, $allow_export, $id]);
        ...
        try {
            $sql = "INSERT INTO products (name, slug, sku, ean, ncm, description, price, price_wholesale, min_wholesale_qty, category_id, image_path, video_url, is_vip, is_manufactured, weight_kg, length_cm, width_cm, height_cm, seo_title, seo_description, brand, gtin, mpn, condition_status, stock_qty, show_on_site, allow_export, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
            $pdo->prepare($sql)->execute([$name, $slug, $sku, $ean, $ncm, $desc, $price, $price_w, $min_w, $cat_id, $image_path, $video_url, $is_vip, $is_manufactured, $weight, $len, $width, $height, $seo_title, $seo_desc, $brand, $gtin, $mpn, $condition, $stock_qty, $show_on_site, $allow_export]);
            $id = $pdo->lastInsertId();
        } catch (PDOException $e) {
                ...
                <div
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; cursor:pointer;"
                        ...
                        <h3 style="margin:0; color:#e056fd; display:flex; align-items:center; gap:10px;">
                            🤖 Cadastro Inteligente com IA (Shopee/ML) <span
                                style="font-size:0.7rem; background:#333; padding:2px 8px; border-radius:10px; color:#fff;">V2.0</span>
                        </h3>
                        <span id="ai-toggle-icon">▼</span>
                    </div>
                    <div id="ai-panel" style="display:none; margin-top:1rem;">
                        <textarea id="ai-json-input" rows="5"
                            placeholder='Cole o JSON do Gemini aqui... Ex: { "name": "Arcade X", "sku": "ARC-01", "ncm": "95045000" ... }'
                            style="width:100%; background:#000; color:#0f0; font-family:monospace; border:1px solid #444; padding:10px;"></textarea>

                        <div style="display:flex; gap:10px; margin-top:10px;">
                            <button type="button" class="btn" style="background:#9b59b6; flex:2;"
                                onclick="processAIData()">✨ Preencher Automaticamente</button>
                            <button type="button" class="btn" style="background:#333; flex:1; border:1px solid #555;"
                                onclick="copyAIPrompt()">📋 Copiar Prompt Completo</button>
                        </div>
                        <p style="color:#666; font-size:0.8rem; margin-top:5px; text-align:center;">
                            Gera dados técnicos completos: NCM, EAN, Peso, Dimensões e SEO.
                        </p>
                    </div>
                </div>
