...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if (isset($_POST['id']) && !empty($_POST['id'])) {
            // Update
            $stmt = $pdo->prepare("UPDATE categories SET name = ?, show_on_site = ? WHERE id = ?");
            $stmt->execute([$name, $show_on_site, $_POST['id']]);
            header("Location: categories.php?msg=updated");
        } else {
            // Insert
            $stmt = $pdo->prepare("INSERT INTO categories (name, show_on_site) VALUES (?, ?)");
            $stmt->execute([$name, $show_on_site]);
        ...
        <?php if (isset($_GET['msg'])): ?>
            ...
            <?php elseif ($_GET['msg'] == 'error_db'): ?>
                <div class="alert alert-danger">❌ Erro no Banco de Dados:
                    <?php echo htmlspecialchars($_GET['debug'] ?? 'Desconhecido'); ?></div>
            <?php endif; ?>
        <?php endif; ?>
