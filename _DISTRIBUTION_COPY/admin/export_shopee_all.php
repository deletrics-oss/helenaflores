<?php
/**
 * Exportador Shopee - Todos os Produtos Ativos
 * Acesso direto via URL para exportar todos os produtos
 */
...
// Busca TODOS os produtos ativos e que permitem exportação
    ...
    foreach ($mapping as $key => $colLetter) {
        ...
        if ($key == 'Capa') {
            ...
            for ($imgIdx = 0; $imgIdx < 8; $imgIdx++) {
                if (isset($imgs[$imgIdx]) && $imgs[$imgIdx] !== '') {
...
// Download
$filename = 'shopee_export_all_' . date('Y-m-d_H-i') . '.xlsx';
