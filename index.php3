...
if (!$is_logged_in && isset($_COOKIE['b2b_access'])) {
    ...
    // Validar formato para evitar injection
...
$sql = "SELECT p.*, c.name as category_name,
        ...
        (SELECT GROUP_CONCAT(image_path) FROM product_variations WHERE product_id = p.id AND image_path IS NOT NULL) as var_images_csv,
        (SELECT GROUP_CONCAT(DISTINCT value SEPARATOR ', ') FROM product_variations WHERE product_id = p.id AND type='Cor') as colors_csv
        FROM products p 
    ...
    if (empty($banner_slides)) {
        $banner_slides = [
            [
                ...
                'title' => 'Controles Arcade Profissionais',
                'subtitle' => 'Precisão de campeonato. Acabamento premium.',
                'link_url' => '?cat=1'
            ]
        ];
    }
    ...
    <div class="hero-slider">
        <?php foreach ($banner_slides as $index => $slide):
            ...
            <div class="slide <?php echo $index === 0 ? 'active' : ''; ?>"
                style="background-image: linear-gradient(to right, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.4) 50%, rgba(0,0,0,0.1) 100%), url('<?php echo $img; ?>'); background-size: cover; background-position: center;">
                    ...
                    <a href="product.php?id=<?php echo $p['id']; ?>" class="product-card">
                        ...
                        <div class="product-info">
                            ...
                            $galleryFull = array_map(function ($img) {
                                return BASE_URL . '/assets/uploads/' . trim($img); }, $galleryArr);
                            ...
                            <div class="prices">
                                ...
                                <?php if ($is_logged_in && isset($p['price_wholesale']) && $p['price_wholesale'] > 0): ?>
                                    <div class="price-tag">
    ...
    <?php if (!$is_logged_in): ?>
        <div id="gatekeeper-overlay">
            <div class="gatekeeper-modal" style="position:relative;">
                ...
                <p>Identifique-se para acessar o catálogo completo e ver preços de atacado.</p>
                ...
                <form id="gatekeeper-form">
                    ...
                    <div class="gk-input-group">
                        ...
                        <select name="source"
                            style="width:100%; padding:0.8rem; background:rgba(255,255,255,0.1); border:1px solid #444; color:#fff; border-radius:4px;">
        ...
        <script>
            ...
            document.getElementById('gatekeeper-form').addEventListener('submit', function (e) {
                ...
                const errorDiv = document.getElementById('gk-error');
                ...
                fetch('login.php', { method: 'POST', body: formData })
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            errorDiv.innerText = data.message || 'Erro ao processar.';
